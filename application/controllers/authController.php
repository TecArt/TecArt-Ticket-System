<?php
    ...
    class authController extends baseController
    {
        ...
        public function login()
        {
            ...
            $this->crmLogon = new crmLogon(null, $this->registry->config);
            $crm_session_id = $this->crmLogon->connect($this->registry->config['username'], $this->registry->config['password']);
            ...
            $_SESSION['crm_session_id'] = $crm_session_id;
    
            // Check username and password
            $this->crm_contact = new crmContacts($crm_session_id, $this->registry->config, $this->registry->baseUrl);
        ...
        {
            $this->crmLogon = new crmLogon($_SESSION['crm_session_id'], $this->registry->config);
            $this->crmLogon->logout();
        ...
        public function change_password()
        {
            if (isset($_POST['old_password']) && isset($_POST['new_password']) && isset($_POST['confirm_password'])) {
                ...
                // Check username and password
                $crm_contact = new crmContacts($_SESSION['crm_session_id'], $this->registry->config, $this->registry->baseUrl);
    
                $contact = $crm_contact->do_contact_authenticate($_SESSION["login_number"], $old_pass);
        ...
        private function init_lists($sqlite, $timestamp)
        {
            ...
            if (empty($ticket_lists)) {
                $lists = $this->crmLogon->get_changed_lists(0);
            } else {
                $lists = $this->crmLogon->get_changed_lists($timestamp);
            }
            ...
            if (count($lists)) {
                ...
                foreach ($lists as $list) {
                    if (in_array($list->list_name, $list_arr)) {
                        $crm_lists = $this->crmLogon->get_ticket_lists($list->list_name);
                        if ($crm_lists === false) {
                            continue;
                        }

                        $sqlite->insert_list($list->list_name, $crm_lists);
                    }
                }
        ...
        private function init_users($sqlite, $timestamp)
        {
            ...
            if (empty($users)) {
                $crm_users = $this->crmLogon->get_changed_users(0);
            } else {
                $crm_users = $this->crmLogon->get_changed_users($timestamp);
            }
        ...
        private function init_contacts($sqlite)
        {
            ...
            if (!empty($action)) { // do insert or update contact info
                ...
                $crm_tickets = new crmTickets($_SESSION['crm_session_id'], $this->registry->config, $this->registry->baseUrl);
    
                // Get crm version
                $crm_version = $this->crmLogon->get_crm_version();
    
                $values = array();
                $values['cid']                = $this->cid;
                $values['name']            = $crm_contact[0]->company;
                ...
                if (isset($this->registry->config['partners']['enabled']) && $this->registry->config['partners']['enabled']) {
                    ...
                    $values['partner_access'] = (isset($crm_contact[0]->$pa_field)) ? $crm_contact[0]->$pa_field : 0;
                    $partner = new crmContract($_SESSION['crm_session_id'], $this->registry->config, $this->registry->baseUrl);
                    $values['partner_contract'] = $partner->has_partner_contract($this->cid);
                }
